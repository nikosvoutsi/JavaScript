...
window.addEventListener('load',()=>{
   ...
   for(i in tasks){
       ...
       if(tasks[i].checked.checked==true){
...
function orderby(value,orderByChecked, orderByDateAdded, orderByDateDue){
   ...
   console.log('checked'+orderByChecked);
   console.log('added'+orderByDateAdded);
   console.log('due'+orderByDateDue)
   if(value==1){
       for(i in orderByDateAdded){
...
function save(){
   tasks = tasks.sort(function (a, b) {
       ...
       console.log(tasks)
       for(i in tasks){
           tasks[i].checked===orderByChecked[i].isChecked;
